...
const App = () => {
	...
	const search = (rows) => {
		console.log(rows);
		if (params.term === 0) return rows;
		return rows.filter(
			(row) => row.email.toLowerCase().indexOf(params.term) > -1
		);
	};
	...
	return (
		<div className='conteiner'>
			...
			<SearchPanel onSearching={search} />

			{params.loading ? (
				<Loader />
			) : (
				<Table data={search(params.userList, praterm)} onSort={onSort} />
			)}
		</div>
	);
};
